...
@section('title', 'Tulis Berita - Admin')
        ...
        <form action="{{ route('admin.berita.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="category" id="category" value="{{ old('category') }}" placeholder="Contoh: Pendidikan, Beasiswa, Tips" required class="mt-2 block w-full rounded-xl border-0 py-2 px-3 text-slate-900 ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-primary-blue sm:text-sm shadow-sm">
                </div>

                <div class="sm:col-span-2">
                    <label for="content" class="block text-sm font-medium text-deep-navy">Isi Berita</label>
                    <textarea name="content" id="content" rows="10" required class="mt-2 block w-full rounded-xl border-0 py-2 px-3 text-slate-900 ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-primary-blue sm:text-sm shadow-sm">{{ old('content') }}</textarea>
                </div>

                <div>
                    <label for="thumbnail" class="block text-sm font-medium text-deep-navy">Thumbnail (Opsional)</label>
